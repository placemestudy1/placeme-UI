...
// SCRUM-27: "fresh roster/readiness" -- the participant count/readiness
// hint must reflect what's actually seated, not just a first-paint snapshot.
test("shows a readiness hint that updates as participants join", async ({ page }) => {
  let participants = [{ userId: "u1", displayName: "Aarav Menon", talkShare: 0 }];
  await mockApi(page, `/api/rooms/${ROOM_ID}/status`, {
    id: ROOM_ID,
    status: "waiting",
    code: "GD-1234",
  ...
  await expect(page.getByText("1 joined · need 1 more to start")).toBeVisible();

  participants = [
    { userId: "u1", displayName: "Aarav Menon", talkShare: 0 },
    { userId: "u2", displayName: "Ishita Rao", talkShare: 0 },
  ];
  await expect(page.getByText("2 joined · ready to start")).toBeVisible({ timeout: 6000 });
});

// SCRUM-27: leave/cancel -- a non-host leaving just frees their own seat
// and returns home immediately, no confirmation needed.
test("a non-host leaving frees their seat and returns home", async ({ page }) => {
  await mockApi(page, `/api/rooms/${ROOM_ID}/status`, {
    id: ROOM_ID,
    status: "waiting",
    code: "GD-1234",
  ...
  await mockApi(page, `/api/rooms/${ROOM_ID}/participants`, { participants: [] });
  let seatDeleted = false;
  await page.route(`**/api/rooms/${ROOM_ID}/seat`, (route) => {
    if (route.request().method() !== "DELETE") return route.fallback();
    seatDeleted = true;
    return route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ id: ROOM_ID, status: "waiting" }),
    });
  });
  ...
  await expect.poll(() => seatDeleted).toBe(true);
});

// SCRUM-27: leave/cancel -- the host cancelling affects everyone still
// waiting, so it's gated behind a confirmation dialog instead of being a
// single click.
test("the host cancelling the room is confirmed before it happens", async ({ page }) => {
  await mockApi(page, `/api/rooms/${ROOM_ID}/status`, {
    id: ROOM_ID,
    status: "waiting",
    code: "GD-1234",
  ...
  await page.route(`**/api/rooms/${ROOM_ID}/seat`, (route) => {
    ...
    return route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ id: ROOM_ID, status: "ended" }),
    });
  });
  ...
  await expect(dialog.getByText("Cancel this room?")).toBeVisible();
  expect(seatDeleted).toBe(false);

  await dialog.getByRole("button", { name: "Cancel room" }).click();
